...
// =========================== Lancer State Priority ===========================
// Move < Attack = Defend < Roll < Hit < HeavyHit < Stun < Compete < Die
// =============================================================================
public class Lancer : Player, IDefendable, ICompetable
{
    [SerializeField] private PlayerAttackController spear;
    [SerializeField] private PlayerDefendController shield;
    ...
    public override void Move()
    {
        ...
        {
            // Run
            if (IsLeftShiftKeyDown && CurrentStamina >= GameConstants.PLAYER_STAMINA_CONSUMPTION_RUN)
            {
                IsRun = true;
                CurrentStamina -= GameConstants.PLAYER_STAMINA_CONSUMPTION_RUN * Time.deltaTime;
    ...
    public override void Roll()
    {
        ...
        if (IsSpaceKeyDown && CurrentStamina >= GameConstants.PLAYER_STAMINA_CONSUMPTION_ROLL)
        {
            CurrentStamina -= GameConstants.PLAYER_STAMINA_CONSUMPTION_ROLL;
            // 키보드 입력 방향으로 회피
            Vector3 moveInput = new Vector3(Input.GetAxisRaw("Horizontal"), 0.0f, Input.GetAxisRaw("Vertical"));
            Vector3 verticalDirection = new Vector3(PlayerCamera.transform.forward.x, 0, PlayerCamera.transform.forward.z);
            Vector3 horizontalDirection = new Vector3(PlayerCamera.transform.right.x, 0, PlayerCamera.transform.right.z);
    ...
    public void CounterSkill()
    {
        ...
        if (IsRKeyDown && CurrentStamina >= GameConstants.PLAYER_STAMINA_CONSUMPTION_COUNTER)
        {
            CurrentStamina -= GameConstants.PLAYER_STAMINA_CONSUMPTION_COUNTER;
            // 키보드 입력 방향으로 공격
            Vector3 moveInput = new Vector3(Input.GetAxisRaw("Horizontal"), 0.0f, Input.GetAxisRaw("Vertical"));
            Vector3 verticalDirection = new Vector3(PlayerCamera.transform.forward.x, 0, PlayerCamera.transform.forward.z);
            Vector3 horizontalDirection = new Vector3(PlayerCamera.transform.right.x, 0, PlayerCamera.transform.right.z);
    ...
    {
        shield.CombatType = COMBAT_TYPE.PERFECT_DEFENCE;
    }
