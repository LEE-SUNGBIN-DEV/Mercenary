...
public class QuickSlots : MonoBehaviour
{
    ...
    private void Update()
    {
        if (UIManager.Instance.UserPanel.enabled)
        {
    ...
    public void LoadPlayerQuickSlots(PlayerSaveData playerSaveData)
    {
        for (int i = 0; i < slots.Length; ++i)
        {
            ...
            {
                slots[i].LoadItem(ItemManager.Instance.FindItemFromList(playerSaveData.quickSlotItemNames[i]), playerSaveData.quickSlotItemCounts[i]);
            }
